...
class Renderer {
    ...
    generateStatic(pageManager) {
        ...
        this.listok.defineFunction('dateTimeFormat', ({ format }, ctx, rootCtx) => {
            return (ctx.dateTime || rootCtx.dateTime).format(format);
        });

        this.listok.defineFunction('getTitle', (ctx) => {
            return ctx.isHome ? ctx.siteTitle : ctx.title;
        });

        this.listok.defineFunction('getDescription', (ctx) => {
            return ctx.isHome ? ctx.siteDescription : ctx.description;
        });
